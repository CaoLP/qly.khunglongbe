<?php
	$this->Html->script(array("product_promote"),array("inline"=>false));
?>
<script>
	var product_promote_url = "<?php echo $this->Html->url(array("controller"=>"product_promotes","action"=>"index"))?>";
</script>
...
<div class="panel-body">
	...
	<div class="col-md-4">
		<div class="panel panel-info">
			...
			<div class="panel-body">
				<div class="list-group" id="list-promote">
					<?php foreach ($promotes as $promote){ ?>
						<a class="list-group-item <?php echo $promote["Promote"]["id"] == $promote_id ? "active" : ""?>" href="javascript:;" data-id="<?php echo $promote["Promote"]["id"]?>">
							<?php echo $promote["Promote"]["name"] . "<br><small>(".date("Y-m-d H:s", strtotime($promote["Promote"]["begin"]))." - ".date("Y-m-d H:s",strtotime($promote["Promote"]["end"])).")</small>"?>
						</a>
					<?php } ?>
				</div>
			</div>
